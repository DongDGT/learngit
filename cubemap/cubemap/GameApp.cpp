...
bool GameApp::InitEffect()
{
	...
	// 创建顶点着色器(2D)
	HR(CreateShaderFromFile(L"HLSL\\cube_VS_2D.cso", L"HLSL\\cube_VS_2D.hlsl", "VS_2D", "vs_5_0", blob.ReleaseAndGetAddressOf()));
	...
	// 创建像素着色器(2D)
	HR(CreateShaderFromFile(L"HLSL\\cube_PS_2D.cso", L"HLSL\\cube_PS_2D.hlsl", "PS_2D", "ps_5_0", blob.ReleaseAndGetAddressOf()));
...
bool GameApp::InitResource()
{
	...
	// 初始化木箱纹理
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"DDS\\logo0.dds", nullptr, m_pWoodCrate.GetAddressOf()));
	// 初始化火焰纹理
	WCHAR strFile[40];
	m_pFireAnims.resize(120);
	for (int i = 1; i <= 120; ++i)
	{
		wsprintf(strFile, L"DDS\\FireAnim\\Fire%03d.bmp", i);
